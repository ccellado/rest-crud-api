...
trait RestApi {
	...
	def parseKeyOrder(key: Option[String] = Some("createdat"), order: Option[String] = Some("desc")) = {
		import ctx._
	...
	def updatePost(id: Int, post: PostEntry) = {
		...
		ctx.transaction {
				implicit ex =>
					ctx.run(query[Blogpost].filter(_.id == lift(id)).update(
